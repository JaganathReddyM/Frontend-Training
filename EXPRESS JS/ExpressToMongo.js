...
function addNewUser(request, response) {
    ...
    // Insert the data into the collection. Check if data insertion is successful.
    udata.save()
      .then((data) => {
        console.log('Insertion Successful...');
        response.json({ message: 'Inserted data successfully' }); // Respond with JSON
      })
      .catch((error) => {
        console.log(error);
        response.status(500).json({ message: 'Unable to insert the data' }); // Respond with JSON and appropriate status code
      });
  }
...
function deleteUser(request, response) {
    ...
    UserData.findOneAndRemove({ 'userId': userId })
        .then((user) => {
            if (!user) {
                response.status(404).send('User not found'); // Use response.status(404).send(...)
            } else {
                console.log('User deleted successfully');
                response.status(200).send('User deleted successfully'); // Use response.status(200).send(...)
            }
        })
        .catch((err) => {
            console.error(err);
            response.status(500).send('Error deleting user'); // Use response.status(500).send(...)
        });
}
